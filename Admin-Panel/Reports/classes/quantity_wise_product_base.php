...
/**
 * Table class for quantity wise product
 */
class quantity_wise_product_base extends ReportTable
{
	...
	public function __construct()
	{
		...
		$this->TableVar = 'quantity_wise_product_base';
		$this->TableName = 'quantity wise product';
		$this->TableType = 'VIEW';
		...
		// qty
		$this->qty = new ReportField('quantity_wise_product_base', 'quantity wise product', 'x_qty', 'qty', '`qty`', 3, -1, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->qty->Sortable = TRUE; // Allow sort
		$this->qty->DefaultErrorMessage = $ReportLanguage->phrase("IncorrectInteger");
		$this->qty->DateFilter = "";
		$this->qty->Lookup = new ReportLookup('qty', 'quantity_wise_product_base', TRUE, 'qty', ["qty","","",""], [], [], [], [], [], [], '`qty` ASC', '');
		$this->qty->Lookup->RenderViewFunc = "renderLookup";
		$this->fields['qty'] = &$this->qty;

		// pro_name
		$this->pro_name = new ReportField('quantity_wise_product_base', 'quantity wise product', 'x_pro_name', 'pro_name', '`pro_name`', 200, -1, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->pro_name->Sortable = TRUE; // Allow sort
		$this->pro_name->DateFilter = "";
		$this->fields['pro_name'] = &$this->pro_name;

		// price
		$this->price = new ReportField('quantity_wise_product_base', 'quantity wise product', 'x_price', 'price', '`price`', 4, -1, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->_sqlFrom <> "") ? $this->_sqlFrom : "`quantity wise product`";
	}
